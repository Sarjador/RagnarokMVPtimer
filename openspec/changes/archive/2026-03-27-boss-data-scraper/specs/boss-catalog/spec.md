...
- **THEN** `bossList.json` is written with all fields matching `BossEntry` schema and no extras

#### Scenario: Catalog remains loadable by BossCatalogService
- **WHEN** the Angular app loads
- **THEN** `BossCatalogService` reads bossList.json without errors and populates the bosses signal
